...
nA = 2000 #number of As
N = 4000 #total number of binding sites in the DNA 
stop_time = 40000
...
for E_ad in E_ad_values:
    ...
    list_A = [-1]*nA #-1 for ubound 1 for bound ---> altrimenti problema con il primo sito 
    ...
    while time_step < stop_time:
        ...
        time_step, list_DNA, list_A = functions_MC_simulation_protein_A.step_MC_2(time_step, list_DNA, list_A,  E_ad, E_aa) #list_empty_DNA, E_ad, E_aa)
        
        nA_bound= functions_MC_simulation_protein_A.count_A(list_A)
        prova = functions_MC_simulation_protein_A.count_consecutive_ones(list_DNA)
        
        if prova != nA_bound:
            raise ValueError(f"Error: prova ({prova}) and nA_bound ({nA_bound}) are not equal")
